...
if ~isfield(parms,'UTC_sat')
    if stamps_flag==1
        ...
        if strcmpi(getparm('insar_processor'),'gamma')
            load ps2.mat
            if strcmpi(getparm('small_baseline_flag'),'n')
                command_str = ['ls ' 'rslc' filesep  datestr(master_day,'yyyymmdd') '*slc.par > temp.master.res'];
            else
                currpwd=pwd;
                if (strcmp(currpwd(end-5:end),'MERGED'))
                    command_str = ['ls ' '../SMALL_BASELINES/SMALL_BASELINES' filesep datestr(master_day,'yyyymmdd') '*' filesep datestr(master_day,'yyyymmdd') '*slc.par > temp.master.res'];
                else
                    command_str = ['ls ' 'SMALL_BASELINES' filesep datestr(master_day,'yyyymmdd') '*' filesep datestr(master_day,'yyyymmdd') '*slc.par > temp.master.res'];

                end
%                 command_str = ['ls ' 'SMALL_BASELINES' filesep datestr(master_day,'yyyymmdd') '*' filesep datestr(master_day,'yyyymmdd') '*slc.par > temp.master.res'];
            end
%             command_str = ['ls ' 'rslc' filesep datestr(master_day,'yyyymmdd') '*' filesep datestr(master_day,'yyyymmdd') '*slc.par > temp.master.res'];
